...
@section('content')
    <div class="space-y-6">
        <div>
            <h2 class="text-2xl font-bold text-white">Tambah Pengumuman</h2>
            <p class="text-slate-400 text-sm mt-1">Buat pengumuman baru untuk jamaah masjid</p>
        </div>

        <div class="bg-slate-800/70 backdrop-blur-sm border border-slate-700 rounded-xl shadow-xl p-6 md:p-8">
            <form action="{{ route('admin.pengumuman-masjid.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
