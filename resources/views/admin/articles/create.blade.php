...
@section('content')
    <div class="space-y-6">
        ...
        <div>
            ...
            <p class="text-slate-400 text-sm mt-1">Tulis artikel baru untuk website masjid</p>
        </div>
            ...
            <form action="{{ route('admin.articles.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8"
                        class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
